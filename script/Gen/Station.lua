...
function Station.GenerateStation (seed)
  ...
  -- Extrude aft section
    ...
    -- Bevel edges and apply surface details (guarded against zero-thickness edge collapse)
    local bevelAmount = math.max(0.05, rng:getUniformRange(0.1, 0.3))
    shape = shape:bevel(bevelAmount)
    shape:greeble(rng, 1, 0.01, 0.03)

    -- Scale station bounds (~30m radius) safely
    local radius = math.max(0.001, shape:getRadius())
    local rcpRadius = 30.0 / radius
