...
-- Localize math functions to prevent GlobalRestrict nil crashes
...
function Station.GenerateStation (seed)
  ...
  -- Un-bypassed procedural station mesh generator
  ...
  -- Fix: Corrected AABB index access from bodyAABB[1] to bodyAABB.lower
  gun1:translate(bodyAABB.lower.x, 0, 0)

  local gun2 = BasicShapes.Prism(2, res)
  gun2:rotate(0, math.pi/2, 0)
  gun2:scale(gunScale.x, gunScale.y, gunScale.z)
  -- Fix: Corrected AABB index access from bodyAABB[2] to bodyAABB.upper
  gun2:translate(bodyAABB.upper.x, 0, 0)
  ...
  -- Scale station bounds so physics collider does not overlap ship at origin
  local rcpRadius = 50.0 / math.max(0.001, shape:getRadius())
