...
void main() {
  ...
  vec2 q1 = p1 + kRadius * safeNormalize(center - p1);
  vec2 q2 = p2 + kRadius * safeNormalize(center - p2);
  vec2 q3 = p3 + kRadius * safeNormalize(center - p3);

  vec2 d1 = safeNormalize(q2 - q1);
  vec2 d2 = safeNormalize(q3 - q2);
  vec2 d3 = safeNormalize(q1 - q3);
  ...
  float glow    = exp(-pow(max(1e-5, 0.25 * edm), 0.75));

  float alpha = 0.0;
  alpha += 0.8 * fill;
  alpha += 0.3 * glow;

  vec4 outCol = alpha * color.w * vec4(2.0 * color.xyz, 1.0);
