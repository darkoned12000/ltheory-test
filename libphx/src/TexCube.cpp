...
TexCube* TexCube_Load (cstr path) {
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    if (!data) {
      if (i == 0) {
        /* First face failed - create a fallback cubemap with magenta faces */
        Warn("TexCube_Load: using fallback for '%s'", path);
        self->size = 1;
        self->format = TexFormat_RGBA8;
        dataLayout = GL_RGBA;
        components = 4;
        uchar fallback[4] = {255, 0, 255, 255};
        for (int j = 0; j < 6; ++j) {
          GLCALL(glTexImage2D(kFaces[j].face, 0, TexFormat_RGBA8, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, fallback))
        }
        TexCube_InitParameters();
        GLCALL(glBindTexture(GL_TEXTURE_CUBE_MAP, 0))
        MemFree(facePath);
      ...
      /* Non-first face failed - reuse face 0's data as fallback */
      Warn("TexCube_Load: using fallback for face %d of '%s'", i, facePath);
      sx = self->size; sy = self->size; lcomponents = components;
      /* Read back face 0 data from GPU */
      int bufSize = self->size * self->size * components;
      data = MemNewArray(uchar, bufSize);
      GLCALL(glGetTexImage(kFaces[0].face, 0, dataLayout, GL_UNSIGNED_BYTE, data))
    }
    if (sx != sy)
