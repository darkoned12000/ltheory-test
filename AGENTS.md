...
- Debug tooling kept env-gated: `PHX_DEBUG_TEXCUBE=1`, `PHX_DEBUG_TEXCUBE_DUMP=<prefix>`, `PHX_DEBUG_DUMP=<frame>` (GameView pipeline PNG dumps), `PHX_SELFTEST_COMPUTE=1` (compute-pipeline round-trip self-test at window creation), `[GL]` context banner at boot.
...
1. **Compute shader plumbing (`.comp` stage)** — ~~THE flagship enabler~~ **DONE (2026-08-23, branch `feat-compute-plumbing`, merged into `modernization-post4_6`)**. Landed: `Shader_LoadCompute(name)` loads `res/shader/compute/<name>.glsl` through the standard preprocessor; `Shader_Dispatch(x,y,z)` + `Shader_MemoryBarrier(bits)` (+ `Shader.Barrier.*` masks via Lua); `GPUBuffer` refcounted SSBO wrapper (`Create/Upload/Download/BindSSBO`); `Cache.Compute(name)` on the Lua side; validator covers `res/shader/compute/*` as standalone programs (114 total); env-gated self-test `PHX_SELFTEST_COMPUTE=1` runs an end-to-end dispatch+SSBO round-trip at boot (`[COMPUTE-TEST] PASS 256/256`). GOTCHA recorded in `selftest.glsl`: uniforms written via `Shader_SetInt` must be declared `int` in GLSL — `glUniform1i` against a `uint` uniform raises INVALID_OPERATION (silent when GLCHECK=0).
2. **GPU particle systems** — ~~explosions, thruster plumes, engine trails~~ **DONE for explosions (2026-08-23, branch `feat-gpu-particles`, merged into `modernization-post4_6`)**. Landed: fixed 128k-sprite pool in SSBOs, simulated by two compute kernels (`particle_spawn` with atomic ring-cursor slot allocation + `particle_simulate` integrate/drag/decay), rendered as ONE instanced attribute-less draw (`vertex/particles` expands quads straight from the buffer via `gl_VertexID`/`gl_InstanceID`; `Draw_DrawArraysInstanced`). Lua API: `GPUParticles.explode(pos, baseVel, scale)` / `.emit{...}` — wired into asteroid destruction. Verify with `PHX_SELFTEST_PARTICLES=1` (boot burst + pool readback -> PASS). GOTCHAS recorded in the shaders: VS pool declaration must mirror the kernels' AoS std430 struct exactly (SoA arrays silently address wrong bytes); group counts passed through FFI must be pre-floored ints (float->uint truncates). Follow-ups: convert Thruster/Dust emitters onto the same pool (legacy CPU billboards still in place and working).
